...
# Jesus Nuker under the GNU General Public Liscense v2 (1991).
...
def main():
    setTitle(f"Jesus Nuker {THIS_VERSION}")
    clear()
    global threads
    global cancel_key
    ...
    if choice == "1":
        ...
        if token.startswith("mfa."):
            print(f'{Fore.RESET}[{Fore.RED}Error{Fore.RESET}] : Just a headsup Jesus wont be able to delete the servers since the account has 2fa enabled')
            sleep(3)
        processes = []
        #get all servers
...
{Fore.RESET}[{Fore.LIGHTGREEN_EX}3{Fore.RESET}]{Fore.LIGHTGREEN_EX} HypeSquad Balance
        ...
        print(f"{Fore.RED}NUKER COMMING SOON\n{Fore.RESET}")
        sleep(4)
        main()
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"{Fore.RED}Woopsie daisy, your python version ({sys.version_info[0]}.{sys.version_info[1]}.{sys.version_info[2]}) is not compatible with Jesus Nuker, please download python 3.7+")
        sleep(5)
        print("exiting. . .")
        sleep(1.5)
        os._exit(0)
